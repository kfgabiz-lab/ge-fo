...
export default function CompanyEventsFeatured({ items }: CompanyEventsFeaturedProps) {
  ...
  const syncPagination = useCallback(
    (swiper: SwiperType) => {
      const group = getSlidesPerGroup(swiper);
      const pages = getPageCount(items.length, group);
      if (loopEnabled) {
        setActivePage(
          Math.min(Math.floor(swiper.realIndex / group), pages - 1),
        );
        return;
      }
    ...
    <section className="company-events-featured">
      <div className="inner">
        <div className="company-events-featured__swiper-area">
          <Swiper
            key={`company-events-featured-${loopEnabled ? "loop" : "slide"}-${isDesktop ? "desktop" : "mobile"}`}
            ...
            onSlideChangeTransitionEnd={syncPagination}
