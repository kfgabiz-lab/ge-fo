...
STEP 1. 마크업 태깅 (fo-slug-analyzer)
        ...
        → ⚠️ **이 STEP은 퍼블리싱된 결과물을 절대 변경하지 않는다.** 기존 태그에 `data-*` 속성을 추가하는 것만 허용되며, 새 태그를 만들거나 className/style/CSS를 수정하는 것은 금지한다. 구조가 데이터 바인딩에 부적합해 보이면 임의로 고치지 말고 그 즉시 질문한다.
...
## 5. 데이터 없음(빈 값 / 매칭 0건) 처리 원칙

`data-slugKey`에 대응하는 실제 데이터가 없거나(dataJson 필드 누락) where 매칭이 0건이어도, 이미 마크업으로 존재하는 **컨테이너/섹션 자체를 조건부 렌더링(`return null`, `{data && ...}` 등)으로 완전히 제거하지 않는다.**

- 컨테이너(제목·설명 등 정적 구조)는 그대로 유지하고, 그 안의 개별 항목(리스트·텍스트·이미지)만 실제 있는 만큼 표시한다.
- **항목이 0개라 리스트 내부가 비어 보이는 것 자체는 정상이며 금지 대상이 아니다.** 예: FAQ 목록이 0건이면 "FAQ" 제목/설명은 그대로 두고 목록만 빈 상태로 두는 것은 허용된다.
- **금지되는 것은 오직 섹션/영역 전체를 통째로 렌더링에서 배제하는 것**이다. 예: `if (!lineUp) return null`처럼 실데이터가 비었다는 이유로 `<section>` 자체를 안 그리는 패턴.
- 이 구분은 2026-07-23 실제 제품상세(product-data) 사례 조사에서 확정됐다 — 상세 근거는 `fo/docs/dev/products-systems/product-data-detail.md` 9번/10-1~10-4/STEP 이력 참고.
