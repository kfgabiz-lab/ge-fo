...
> 상태: 구현 완료(BE 변경 없음, press/blog용 범용 로직 재사용). `tsc` 통과, API 레벨 검증 완료. 실데이터 게시상태 조건 만족 0건이라 화면은 현재 빈 목록으로 보임(6-2 참고, 코드 문제 아님) — 사용자가 BO에서 테스트 데이터 추가 후 재확인 예정 / **Month·Year 필터 옵션 조정: 구현·검증 완료(2026-07-21, 브라우저 실검증 완료 — 8절 참고)** / **정렬 A-Z/Z-A 확장 설계중(2026-07-21, 승인 대기 — 9절 참고)** / **상세조회·pager(이전글/다음글) 성능개선 설계중(2026-07-21, 승인 대기 — 10절 참고)**
...
## 9. 정렬 확장 — A-Z / Z-A (2026-07-21 신규 스코프, 설계중·승인 대기)
...
| 일자 | 내용 |
|---|---|
| 2026-07-21 | 정렬 A-Z/Z-A 확장(9절) STEP1~2 설계 확정, fo-dev-doc-writer가 문서화(상태: 설계중, 승인 대기). press-data.md와 동일 공용 컴포넌트(`CompanyFeedListToolbar`) 변경이라 두 문서에 동시 반영. API 확인은 "기존 활용 가능(잠정)"이나 최종 검증은 STEP3(fo-be-analyzer) 예정 — `#개발` 승인 전까지 개발 착수 없음 |
...
| 인접 titleField | `articles.title` (dot-notation bare 래퍼 — 9절에서 이미 title 정렬 경로로 확정된 표기와 동일) |
