...
> 상태: 개발완료 (STEP1~7 완료 후, 2026-07-15 `flattenPageDataItem` 기반 재작업, 2026-07-21 필드명 스키마 재확정 재작업 — 하단 STEP 이력 참고)
...
- **필드명/코드값 체계 (2026-07-21 재확정).** bo `page_template`(id=87 `faq-detail`, id=86 `faq-list` — FAQ 등록/수정·목록 화면 둘 다) `config_json`을 직접 조회해 fieldKey를 전수 확인한 결과, **contentKey는 `faq`, 필드는 전부 snake_case**(`main_category`/`markets`/`question`/`answer`/`is_visible`/`product`)로 확정됐다. 실제 구현은 **콘텐츠 키 이름에 의존하지 않는 공통 유틸 `flattenPageDataItem`**(`fo/src/lib/pageData.ts`)로 dataJson을 flat row로 변환한 뒤, root 필드 `question`(질문 텍스트)/`answer`(답변 텍스트)를 그대로 `question`/`answer`로 매핑해 사용한다(`fo/src/app/markets/data/marketsFaqData.ts`). where 조건에 쓰는 필드명은 snake_case `main_category`/`is_visible`/`markets`다. (이전 버전 문서는 camelCase `mainCategory`/`isVisible`+`title`→question 매핑으로 잘못 기재돼 있었고, 그 상태로 구현돼 있어 실제로는 라이브 데이터와 전혀 매칭되지 않았다 — 하단 STEP 이력 "재작업(2026-07-21)" 참고.)
...
- 페이지별 호출 (Query Params, bo 빌더 저장 필드명 그대로 snake_case):
...
- **동작 근거**: `PageDataService.appendWhereConditions()`의 `eq_` 단순키 분기가 최상위 + 1단계 중첩 object를 `jsonb_each ... EXISTS`로 함께 탐색한다. bo `faq-detail`/`faq-list` 빌더 템플릿(`page_template` id=87/86)이 저장하는 필드명(snake_case `main_category`/`is_visible`/`markets`)과 정확히 일치해야 매칭된다.
- **응답 → FE 매핑**: `flattenPageDataItem(item)`으로 dataJson을 flat row로 변환(콘텐츠 키 이름 무관) → `row.question` → `question`, `row.answer` → `answer`로 매핑. 절대 `dataJson.{콘텐츠키}.question`처럼 특정 콘텐츠 키·필드명을 하드코딩해서 직접 언랩하지 않는다(`fo/docs/fo-data-binding-가이드.md`의 공통 규칙 그대로 적용).
- **markets 필드는 단일 select(2026-07-21 확인)** — press/blog/articles-data와 달리 CSV 다중값이 아니다. bo `faq-detail`(`page_template` id=87) config_json에 `{"type":"select","fieldKey":"markets","codeGroupCode":"MARKETS",...}`로 정의돼 있고, `FieldRenderer.tsx`의 `case "select":`는 네이티브 `<select>`로 스칼라 값 하나만 기록한다(체크박스 다중선택 패턴과 다름). 그래서 `has_markets_` prefix가 아니라 `eq_markets=` 정확일치로 조회하는 현재 방식이 맞다.

### ⚠️ 스키마 세대 혼재 — 실제 구현이 대상으로 삼는 세대 (2026-07-21 재확정)
...
2. content key `faqForm` — `faqForm.{faqMainCategory, faqTitle, isVisible, markets}` (필드명 자체가 다름, markets 키 자체가 없는 레코드도 존재)
3. content key `faqForm` — `faqForm.{mainCategory, title, isVisible, markets, answer}` (camelCase, 질문 텍스트 필드명이 `title`)

**1번 세대(`faq.{main_category, markets, is_visible, question, answer}`)가 bo `faq-detail`/`faq-list` 빌더 템플릿(`page_template` id=87/86 `config_json`)이 지금 실제로 저장하는 스키마임을 직접 확인했다** — 즉 1번이 "여러 세대 중 하나"가 아니라 **현재도 유효한 정본 스키마**다. 2·3번은 과거 템플릿 세대의 잔존 데이터로, 지금 bo 화면에서 신규 등록해도 재현되지 않는다.

**현재 구현(`eq_main_category`/`eq_is_visible`/`eq_markets` + `row.question`→`question`)은 1번 세대만 매칭 대상으로 삼는다.** 2026-07-15~07-20 사이에는 3번 세대(camelCase `mainCategory`/`title`) 기준으로 잘못 구현돼 있어서 정본 스키마인 1번 데이터가 전혀 매칭되지 않았다(그 결과 6개 markets 페이지 FAQ가 전부 빈 목록이었다 — 하단 STEP 이력 "재작업(2026-07-21)" 참고). 2·3번 세대로 입력된 레코드는 여전히 노출되지 않으며, 필요하면 BO에서 1번 세대 필드명으로 재저장해야 한다(코드가 아닌 데이터 정리 과제).
...
> 아래는 실제 구현이 매칭 대상으로 삼는 스키마(bo `faq-detail`/`faq-list` 빌더가 지금 저장하는 정본 스키마, `faq` content key + snake_case 필드명)를 기준으로 한 예시다. `flattenPageDataItem`을 거치면 `main_category`/`question`/`is_visible`/`markets`/`answer`가 root로 flat 병합되어 `row.question`/`row.answer`로 접근 가능하다.
...
3. `main_category`/`markets`/`is_visible`/`question`/`answer`(snake_case, `faq` content key) 필드명 체계로 조회하도록 구현 확정(2026-07-21 재확정) — 3번 API 확인 섹션 참고. bo `page_template`(id=87/86) config_json으로 직접 확인한 정본 스키마이며, 다른 세대(`faqForm` + camelCase, 또는 `faqForm.faqMainCategory`/`faqTitle`)로 입력된 레코드는 노출되지 않는다.
...
6. (2026-07-21 갱신) 정본 스키마(`faq.*` snake_case) 기준으로 라이브 레코드가 실제로 존재한다 — 예: id=1288(Data Center, 공개). 이전 버전(3번 세대 camelCase 기준)에서는 "라이브 레코드 0건이라 빈 목록"이라고 설명했지만, 실제로는 구현이 잘못된 세대를 보고 있었을 뿐 데이터 자체는 있었다. 재작업 후 SSR HTML로 노출 확인 완료.
...
| 재작업 | (사용자 요청 정밀분석 → 직접 확인) | 2026-07-21 | `/markets` 6페이지 전수 검증 중 재발견: 07-15 재작업이 잘못된 세대(camelCase 3번)를 기준으로 삼아 실제로는 여전히 0건 노출이었음. bo `page_template`(id=87 `faq-detail`, id=86 `faq-list`) `config_json`을 직접 조회해 fieldKey 전수 확인 → 정본 스키마가 `faq.{main_category, markets, question, answer, is_visible}`(snake_case, 1번 세대)임을 확정. `marketsFaqData.ts`의 where를 `eq_main_category`/`eq_is_visible`/`eq_markets`로, 매핑을 `row.question`→question으로 수정. `markets` 필드가 CSV 다중값이 아니라 단일 select(`FieldRenderer.tsx` `case "select"`)임도 함께 확인(press/blog/articles-data와 다름 — 그쪽은 checkbox 다중선택+CSV 확인). 수정 후 SSR HTML(`localhost:3002/markets/data-center`)에서 id=1288 정상 노출 확인 |
